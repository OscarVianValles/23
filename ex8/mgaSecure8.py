...
class Delivery(ABC):
    ...
    @abstractmethod
    def estimatedDeliveryDate(self, processDate: date):
        pass
...
class FreeDelivery(Delivery):
    ...
    def estimatedDeliveryDate(self, processDate: date) -> float:
        return processDate + timedelta(days=30)
...
class Shipment:
    ...
    def shipmentDetails(self) -> str:
        ...
        r += (
            "PRICE WITH DELIVERY FEE : P"
            + str(self.totalPrice() + self._delivery.deliveryFee())
            + "\n"
        )
        r += "ESTIMATED DELIVERY DATE: " + str(
            self._delivery.estimatedDeliveryDate(self._processDate)
        )
        return r
...
class FreeShipment(Shipment):
    def newDelivery(self, location) -> Delivery:
        return FreeDelivery(location)
